...
bool Game::OnCommonEvent(const irr::SEvent& event) {
	...
	case irr::EET_GUI_EVENT: {
		...
		case irr::gui::EGET_ELEMENT_HOVERED: {
			if (event.GUIEvent.Caller->getType() == EGUIET_EDIT_BOX) {
				SetCursor(event.GUIEvent.Caller->isEnabled() ? ECI_IBEAM : ECI_NORMAL);
				return true;
			}
			if (event.GUIEvent.Caller == imgCard && is_building && !is_siding) {
				SetCursor(ECI_HAND);
				return true;
			}
			break;
		...
		case irr::gui::EGET_ELEMENT_LEFT: {
			if (event.GUIEvent.Caller->getType() == EGUIET_EDIT_BOX || event.GUIEvent.Caller == imgCard) {
				SetCursor(ECI_NORMAL);
				return true;
			}
			break;
		}
		...
		case irr::gui::EGET_CHECKBOX_CHANGED: {
			...
			case CHECKBOX_AUTO_SEARCH: {
				gameConf.auto_search_limit = chkAutoSearch->isChecked() ? 0 : -1;
				if (is_building && !is_siding)
					deckBuilder.InstantSearch();
				return true;
				break;
			}
			case CHECKBOX_MULTI_KEYWORDS: {
				gameConf.search_multiple_keywords = chkMultiKeywords->isChecked() ? 1 : 0;
				if (is_building && !is_siding)
					deckBuilder.InstantSearch();
				return true;
				break;
			...
			case CHECKBOX_DISABLE_CHAT: {
				bool show = is_building ? false : !chkIgnore1->isChecked();
				wChat->setVisible(show);
				if (!show)
					ClearChatMsg();
				return true;
				break;
			...
			case CHECKBOX_LFLIST: {
				...
				cbLFlist->setSelected(gameConf.use_lflist ? gameConf.default_lflist : cbLFlist->getItemCount() - 1);
				cbHostLFlist->setSelected(gameConf.use_lflist ? gameConf.default_lflist : cbHostLFlist->getItemCount() - 1);
				deckBuilder.filterList = &deckManager._lfList[cbLFlist->getSelected()].content;
				return true;
				break;
			}
			}
		...
		case irr::gui::EGET_COMBO_BOX_CHANGED: {
			...
			case COMBOBOX_LFLIST: {
				gameConf.default_lflist = cbLFlist->getSelected();
				cbHostLFlist->setSelected(gameConf.default_lflist);
				deckBuilder.filterList = &deckManager._lfList[gameConf.default_lflist].content;
				return true;
				break;
			}
			}
		...
		case irr::gui::EGET_LISTBOX_CHANGED: {
			switch (id) {
			case LISTBOX_LOG: {
				int sel = lstLog->getSelected();
				if (sel != -1 && (int)logParam.size() >= sel && logParam[sel]) {
					ShowCardInfo(logParam[sel]);
				}
				return true;
				break;
			}
		...
		case irr::gui::EGET_SCROLL_BAR_CHANGED: {
			...
			case SCROLL_CARDTEXT: {
				if (!scrCardText->isVisible()) {
					return true;
					break;
				}
				u32 pos = scrCardText->getPos();
				SetStaticText(stText, stText->getRelativePosition().getWidth() - 25, guiFont, showingtext, pos);
				return true;
				break;
			}
			case SCROLL_VOLUME: {
				gameConf.sound_volume = (double)scrSoundVolume->getPos() / 100;
				gameConf.music_volume = (double)scrMusicVolume->getPos() / 100;
				soundManager.SetSoundVolume(gameConf.sound_volume);
				soundManager.SetMusicVolume(gameConf.music_volume);
				return true;
				break;
			}
			case SCROLL_TAB_HELPER: {
				rect<s32> pos = tabHelper->getRelativePosition();
				tabHelper->setRelativePosition(recti(0, scrTabHelper->getPos() * -1, pos.LowerRightCorner.X, pos.LowerRightCorner.Y));
				return true;
				break;
			}
			case SCROLL_TAB_SYSTEM: {
				rect<s32> pos = tabSystem->getRelativePosition();
				tabSystem->setRelativePosition(recti(0, scrTabSystem->getPos() * -1, pos.LowerRightCorner.X, pos.LowerRightCorner.Y));
				return true;
				break;
			}
			}
	...
	case irr::EET_KEY_INPUT_EVENT: {
		...
		case irr::KEY_KEY_R: {
			if (gameConf.control_mode == 0
				&& !event.KeyInput.PressedDown && !HasFocus(EGUIET_EDIT_BOX)) {
