		...
		case irr::gui::EGET_BUTTON_CLICKED: {
			...
			case BUTTON_DM_OK: {
				...
				case BUTTON_RENAME_DECK: {
					...
					ChangeCategory(catesel);
					RefreshDeckList();
					for(irr::u32 i = 0; i < mainGame->lstDecks->getItemCount(); i++) {
			...
			case BUTTON_SIDE_OK: {
				...
				for(size_t i = 0; i < deckManager.current_deck.main.size(); ++i)
					BufferIO::Write<uint32_t>(pdeck, deckManager.current_deck.main[i]->code);
				for(size_t i = 0; i < deckManager.current_deck.extra.size(); ++i)
					BufferIO::Write<uint32_t>(pdeck, deckManager.current_deck.extra[i]->code);
				for(size_t i = 0; i < deckManager.current_deck.side.size(); ++i)
					BufferIO::Write<uint32_t>(pdeck, deckManager.current_deck.side[i]->code);
...
void DeckBuilder::RefreshDeckList() {
	...
	lstDecks->clear();
	for(irr::u32 i = 0; i < mainGame->cbDBDecks->getItemCount(); ++i) {
		lstDecks->addItem(mainGame->cbDBDecks->getItem(i));
	}
}
