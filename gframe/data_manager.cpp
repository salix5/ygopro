...
" texts.str9, texts.str10, texts.str11, texts.str12, texts.str13, texts.str14, texts.str15, texts.str16 FROM datas INNER JOIN texts USING(id)";
...
bool DataManager::Error(sqlite3* pDB, sqlite3_stmt* pStmt) {
	if (const char* msg = sqlite3_errmsg(pDB))
		std::snprintf(errmsg, sizeof errmsg, "sqlite3_errmsg: %s", msg);
	else
		errmsg[0] = '\0';
	sqlite3_finalize(pStmt);
	return false;
...
std::vector<uint32_t> DataManager::GetSetCodes(std::wstring setname) const {
	...
	for(auto& [code, setnameString] : _setnameStrings) {
		size_t start = 0;
