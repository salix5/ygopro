		...
		case irr::EMIE_MOUSE_MOVED: {
				...
				if(mcard) {
					...
					if(mcard->code) {
						...
						if(mcard->location & 0xe) {
							...
							for (auto [counter, value] : mcard->counters) {
							...
							for(auto [desc_hint, value] : mcard->desc_hints) {
								myswprintf(formatBuffer, L"\n*%ls", dataManager.GetDesc(desc_hint));
								str.append(formatBuffer);
							}
							should_show_tip = true;
			...
			if(mplayer != hovered_player) {
				if(mplayer >= 0) {
					...
					for(auto [desc_hint, value] : mplayer_hints) {
						myswprintf(formatBuffer, L"\n*%ls", dataManager.GetDesc(desc_hint));
						str.append(formatBuffer);
					}
...
void ClientField::ShowCardInfoInList(ClientCard* pcard, irr::gui::IGUIElement* element, irr::gui::IGUIElement* parent) {
	...
	if (pcard->location != LOCATION_DECK) {
		...
		for (auto [desc_hint, value] : pcard->desc_hints) {
			myswprintf(formatBuffer, L"\n*%ls", dataManager.GetDesc(desc_hint));
			str.append(formatBuffer);
		}
