...
bool DuelClient::ClientAnalyze(unsigned char* msg, int len) {
	...
	case MSG_SELECT_POSITION: {
		...
		if(positions & 0x1) {
			...
			mainGame->btnPSAU->setRelativePosition(irr::core::rect<s32>(startpos, 45, startpos + 140, 185));
			mainGame->btnPSAU->setVisible(true);
			startpos += 145;
		} else mainGame->btnPSAU->setVisible(false);
		if(positions & 0x2) {
			mainGame->btnPSAD->setRelativePosition(irr::core::rect<s32>(startpos, 45, startpos + 140, 185));
		...
		if(positions & 0x4) {
			...
			mainGame->btnPSDU->setRelativePosition(irr::core::rect<s32>(startpos, 45, startpos + 140, 185));
			mainGame->btnPSDU->setVisible(true);
			startpos += 145;
		} else mainGame->btnPSDU->setVisible(false);
		if(positions & 0x8) {
			mainGame->btnPSDD->setRelativePosition(irr::core::rect<s32>(startpos, 45, startpos + 140, 185));
	...
	case MSG_ANNOUNCE_NUMBER: {
		...
		if(quickmode) {
			...
			mainGame->btnANNumberOK->setRelativePosition(irr::core::rect<s32>(20, 195, 210, 230));
			...
			mainGame->btnANNumberOK->setRelativePosition(irr::core::rect<s32>(80, 60, 150, 85));
			irr::core::recti pos = mainGame->wANNumber->getRelativePosition();
